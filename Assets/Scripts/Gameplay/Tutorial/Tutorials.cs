...
namespace RM_EDU
{
    ...
    public class Tutorials : MonoBehaviour
    {
        ...
        {  
            ...
            {
                // Load the pages.
                new EDU_Page(title, titleKey, "Welcome to the Energy Defense Unit (EDU)! In this training simulation, your mission is to stop aliens that are trying to steal Earth's natural resources. There are two stage types in this simulation game: action (blue) and knowledge (green). You'll get more details on both later.", "trl_intro_txt_00", tutorialsUI.textBox.stageTypesSprite),
        ...
        public TutorialInfo GetFirstActionIntroTutorialInfo()
        {
            ...
            List<Page> pages = new List<Page>
            {
                ...
                new EDU_Page(title, titleKey, "On the left side of the field are lane blasters. A lane blaster destroys all enemies in its row upon an enemy touching it, but this destroys the lane blaster in the process. You cannot restore lost lane blasters, but you always get them back at the start of a stage.", "trl_firstActionIntro_txt_02", tutorialsUI.textBox.actionDefenseLaneBlasterSprite),
                new EDU_Page(title, titleKey, "To place a unit on the field you need energy. Some energy is generated periodically, but that won’t be enough to defeat the enemies. To get more energy, you need to create generator units, which can be found in the bottom left.", "trl_firstActionIntro_txt_03"),
                new EDU_Page(title, titleKey, "The energy needed to create a unit is displayed on its unit button. When you have a unit selected, it’ll be displayed in the bottom middle, and spots on the field will be highlighted to show where it can go. With all that covered, please place a unit on the field.", "trl_firstActionIntro_txt_04", tutorialsUI.textBox.actionUnitSelectedSprite),
            };
        ...
        public TutorialInfo GetFirstActionFirstKillTutorialInfo()
        {
            ...
            List<Page> pages = new List<Page>
            {
                ...
                new EDU_Page(title, titleKey, "Relatedly, the arrows on a unit button light up if you have energy to create the unit they're pointing towards in a nearby row. A unit button's arrows remain dark if they have no unit to point to.", "trl_firstActionFirstKill_txt_01", tutorialsUI.textBox.actionUnitButtonsSprite),
                ...
                new EDU_Page(title, titleKey, "The energy disable button toggles the energy flow to defense units. When the flow is disabled, defense units cannot use attacks that require energy. Any defense units affected will also go dark.", "trl_firstActionFirstKill_txt_05", tutorialsUI.textBox.energyBlockSprite),
        ...
        public TutorialInfo GetHydroTutorialInfo()
        {
            ...
            {
                // Load the pages.
                new EDU_Page(title, titleKey, "This stage uses hydro, which uses the flow of water for generating energy. Hydro generators can only be placed in rivers and cannot be placed next to other hydro generators. They also cannot be placed next to wind generators that are in the water.", "trl_hydro_txt_00", tutorialsUI.textBox.hydroSprite),
